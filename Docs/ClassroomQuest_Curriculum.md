...
| **Social Studies/Values:** Local history, basic civics. | Map-based treasure hunts, “help the town” missions requiring resource allocation decisions. | `MapKit` overlays for key locations. Resource allocation implemented with `Slider` controls and real-time feedback via `Charts`. |
...
| **Social Studies/Values:** Maps, cultures, community decision making. | Cultural exchange quests collecting artifacts, branching narratives with moral choices. | Artifact collections displayed as `Grid` galleries. Narrative choices stored as `DecisionNode` objects; endings unlocked recorded via Core Data. |
...
| **Social Studies/Values:** Local/state history, economics basics. | City builder missions, trade simulations, cause/effect timelines. | City builder grid with `LazyVGrid` and resource counters. Timelines built with `ScrollView` horizontal indicators. |
